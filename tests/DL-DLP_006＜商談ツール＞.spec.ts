...
await page.locator('#vehicle_info').getByRole('link', { name: 'ソートのクリア' }).click();
  ...
  await page.waitForTimeout(5000);
// 購入検討車輛＞新車価格＞検索結果一覧＞ページングの確認
await page.locator('#vehicle_info').getByRole('link', { name: '次へ>>' }).click();
await expect(page.locator('#vehicle_info').getByText('件中 51 -')).toBeVisible({ timeout: 15000 });
await page.locator('#vehicle_info').getByRole('link', { name: '<<前へ' }).click();
await expect(page.locator('#vehicle_info').getByText('件中 1 -')).toBeVisible({ timeout: 15000 });
//await page.locator('#vehicle_info').getByRole('link', { name: '2' }).click();
// ⭕️ 修正後：裏側のシステム名で探したあと、さらに文字が「2」のものに絞り込む！
await page.locator('a[onclick*="purchase_search&purchase_p=2"]')
  .filter({ hasText: '2' }) // 🌟 追加：文字に「2」が含まれる方を選ぶ！
  .click();
await expect(page.locator('#vehicle_info').getByText('件中 51 -')).toBeVisible({ timeout: 15000 });
//await page.locator('#vehicle_info').getByRole('link', { name: '1' }).click();
// ⭕️ 修正後：裏側のシステム名で探したあと、さらに文字が「1」のものに絞り込む！
await page.locator('a[onclick*="purchase_search&purchase_p=1"]')
  .filter({ hasText: '1' }) // 🌟 追加：文字に「1」が含まれる方を選ぶ！
  .click();
await expect(page.locator('#vehicle_info').getByText('件中 1 -')).toBeVisible({ timeout: 15000 });
