...
// 2. 画像から「title」を引っこ抜く！
let originalValue = await ImageLocator.getAttribute('title');
console.log(`💡 確認：1件目の燃料アイコンのシステム値は [${originalValue}] です！`);
// 3. 🌟 発掘した正式名称「ガソリン（ハイブリッド）」と完全一致するかチェック！
//expect(fuelTitle).toBe('ガソリン（ハイブリッド）');
//console.log('✅ 1件目のデータが「HV（ガソリン（ハイブリッド））」であることを確認しました！完璧です！');
/*let targetCell = page.locator('#t_result_area > tbody > tr:nth-child(1) > td.col_fuel_text > img');
// 1. ソート前の「元の値」を取得する
//let originalValue = await targetCell.innerText();
console.log(`💡 「型式」ソート前の1件目の値: [${originalValue}]`);*/
...
// ソートクリア後の、1件目の値を取得する
let clearValue = await ImageLocator.getAttribute('title');
...
// フィルタ適用後の1件目の値を取得する
let filteredValue = await ImageLocator.getAttribute('title');
...
// フィルタ解除後の1件目の値を取得する
let clearedValue = await ImageLocator.getAttribute('title');
console.log(`💡 フィルタ解除後の1件目の値: [${clearedValue}]`);
...
/*
//await page.getByRole('link', { name: '▼' }).nth(18).click();
await page.locator('a[onclick*="KAWA Susie"][onclick*="DESC"]').click();
// 🌟 フライング防止＆ローディング待ち
await page.waitForTimeout(9500); 
await expect(page.locator('#loadingdialog')).toBeHidden({ timeout: 30000 });
// 🌟 安全対策：画像があるかカウントしてから取得する！（降順）
