...
//DLP回帰テスト-006＜商談ツール＞と009＜注文販売＞先に実行済み
test('商談履歴', async ({ page }) => {
...
const textFilePath = './downloads/006-kaitorishitadoriPrice.txt';
// ==========================================
// 🌟 ファイルの中身を読み取って、文字列として変数に代入する！
// ==========================================
// 第2引数の 'utf-8' を忘れずに付けるのがポイントです！
const callKaitoriKakaku = fs.readFileSync(textFilePath, 'utf-8');
// 読み込めたか確認するためにログに出してみる
console.log(`💡 ファイルから読み込んだ価格データは: [${callKaitoriKakaku}] です！`);

// 金額内訳：計（input）の値が前回編集した「xxx」であることを確認する！
await expect(page.locator('#vehicle_breakdown > table > tbody > tr:nth-child(9) > td.v_bd02.v_bd_total')).toContainText(callKaitoriKakaku);
console.log('金額内訳：計（input）の値が前回編集した：',callKaitoriKakaku);
...
const textFilePath2 = './downloads/009-chumonhanbaiPrice.txt';
// ==========================================
// 🌟 ファイルの中身を読み取って、文字列として変数に代入する！
// ==========================================
// 第2引数の 'utf-8' を忘れずに付けるのがポイントです！
const callchumonhanbai = fs.readFileSync(textFilePath2, 'utf-8');
// 読み込めたか確認するためにログに出してみる
console.log(`💡 ファイルから読み込んだ価格データは: [${callchumonhanbai}] です！`);

// 金額内訳：計（input）の値が前回編集した「xxx」であることを確認する！
await expect(page.locator('#vehicle_breakdown > table > tbody > tr:nth-child(9) > td.v_bd02.v_bd_total')).toContainText(callchumonhanbai);
console.log('金額内訳：計（input）の値が前回編集した：',callchumonhanbai);
...
//ダウンロードしたcsvファイルを削除（後処理3）
const filePath7 = './downloads/006-kaitorishitadoriPrice.txt';
// 🌟 ファイルが存在するかチェックしてから削除する（エラー防止の鉄則！）
if (fs.existsSync(filePath7)) {
  fs.unlinkSync(filePath7); // ここでファイルを物理削除！
  console.log(`💡 古いファイル [${filePath7}] をお掃除（削除）しました！`);
} else {
  console.log(`💡 [${filePath7}] は存在しないため、削除をスキップしました。`);
}
//ダウンロードしたcsvファイルを削除（後処理4）
const filePath8 = './downloads/009-chumonhanbaiPrice.txt';
// 🌟 ファイルが存在するかチェックしてから削除する（エラー防止の鉄則！）
if (fs.existsSync(filePath8)) {
  fs.unlinkSync(filePath8); // ここでファイルを物理削除！
  console.log(`💡 古いファイル [${filePath8}] をお掃除（削除）しました！`);
} else {
  console.log(`💡 [${filePath8}] は存在しないため、削除をスキップしました。`);
}
